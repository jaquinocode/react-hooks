...
I did everything except for the error boundary parts so I won't talk about those
here.

Some things I learned:

- How HTTP works (not related to exercises, but I recalled the mneumonic for it
  in my paper notes. I remembered the mail/post office mneumonic.)
- useReducer's awesomeness. Hard to know something's usefulness until you wind
  up w/ weird code & then have useReducer clean that up. Which definitely
  happened here. It's great for abstracting away state update logic so you don't
  have to manually handle more complex state updates, especially if those
  different individual state updates are relational to each other.
- I learned a way to make a component that fetches from a server in a way that
  is considered 'good' or follows good practices, as far as I know so far. Seems
  a lot more straightforward now!
- The nature of async/await syntax. And how to use it w/ useEffect.
- The great idea of using a status to simplify our logic when making a data
  fetching component.
- Learning that set state updates only batch inside React event handlers, but
  WON'T batch w/ everything else like promises, async functions, native event
  handlers (event handlers using the imperative DOM API rather than using the
  React event handler attributes in JSX), etc..
  - This means that putting two set state functions next to each other, one
    right in the next line of code after the 1st one, will actually render the
    component twice, rather than just waiting a little and rendering the
    component once. This can lead to bugs, as noted in Kent's videos for this
    exercise, and it leads to unecessary renders.
    - A fix for this is to just make sure you set your state carefully in a good
      order, or use useReducer so the problem won't exist anymore.
  - This problem is only temporary. In React 18, you can manually opt-in to the
    new behavior where this is fixed and basically all state updates will batch.
    In future updates, it'll be the standard behavior.
- A reminder of the problems that come w/ putting all state variables into one
  state object.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=06%3A%20useEffect%3A%20HTTP%20requests&em=jaquinoemail%40gmail.com).
